...
qmail-send logic. That is: email for domains not in C<virtualdomains> or
C<locals> is I<accepted>, so it can be relayed to another server. This makes
the solutions (almost) drop-in compatible with most qmail setups.
