...
subtest '0x14 (ezmlm) accepts normal sender, denies null sender' => sub {
    ...
    like $msg, qr/mailing lists do not accept null senders/i,
        'mentions null senders';
...
subtest '0xff -> DECLINED, "not local" logged' => sub {
    ...
    ok( ( grep { $_->[1] =~ /not local/i } $log->entries ),
       'log entry mentions not local' );
};
...
subtest 'unknown nonzero status -> DECLINED, "unknown" logged' => sub {
    ...
    ok( ( grep { $_->[1] =~ /unknown/i } $log->entries ),
       'log entry mentions unknown' );
};
